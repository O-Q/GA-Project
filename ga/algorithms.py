...
class BaseGeneticAlgorithm(abc.ABC):
    ...
    def reproduce(self, survivors, p_crossover, target_size=None, n_point_crossover=1):
        """
        ...
        while num_survivors + len(offspring) < target_size:
            ...
            c1 = random.choice(survivors).copy()
            ...
            if random.random() < p_crossover:
                ...
                crosspoints = random.sample(range(0, c1.length), n_point_crossover)
                crosspoints.sort()
                c1.crossover(c2, crosspoints)
                offspring.append(c1)
                if len(offspring) + num_survivors < target_size:
                    offspring.append(c2)
            else:
                offspring.append(c1)
        return offspring
